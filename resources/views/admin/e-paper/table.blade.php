        ...
        @if(count($result) > 0) 
            @foreach($result as $row) 
                ...
                <tr>
                    <td>{{$i}}</td>
                    <td>
                        <img src="{{ url('uploads/e-paper/'.$row->image)}}" class="me-75" height="50" width="50" alt="{{$row->name}}" onerror="this.onerror=null;this.src=`{{ asset('uploads/no-image.png') }}`" />
                    </td>
                    <td>
                                ...
                                <form class="add-new-record pt-0 row g-2" id="edit-record" action="{{url('admin/update-e-paper')}}" method="POST" enctype="multipart/form-data" onsubmit="return validateEpaper();"> @csrf 
                                    ...
                                    <div class="col-sm-12">
                                        ...
                                        <div class="d-flex">
                                            ...
                                            <a target="_blank" class="rounded me-50" href="{{ asset('uploads/e-paper/pdf')}}/{{$row->pdf}}"><i class="ti ti-download me-1 margin-top-negative-4"></i></a>
                                            @endif
                                            <!-- <img src="{{ asset('uploads/live-news')}}/{{$row->image}}" id="image-preview_{{$row->id}}" class="rounded me-50" alt="profile image" height="80" width="80" onerror="this.onerror=null;this.src=`{{ asset('uploads/no-image.png') }}`" /> -->
                                    ...
                                    <div class="col-sm-12">
                                        ...
                                        <div class="d-flex">
                                            <img src="{{ asset('uploads/e-paper')}}/{{$row->image}}" id="image-preview_{{$row->id}}" class="rounded me-50" alt="profile image" height="80" width="80" onerror="this.onerror=null;this.src=`{{ asset('uploads/no-image.png') }}`" />
                                            <div class="mt-75 ms-1">
                                            <label class="btn btn-primary me-75 mb-0" for="change-picture_{{$row->id}}">
                                                <span class="d-none d-sm-block">{{__('lang.admin_upload')}}</span>
                                                <input class="form-control" type="file" id="change-picture_{{$row->id}}" hidden accept="image/png, image/jpeg, image/jpg" name="image" onclick="showImagePreview('change-picture_{{$row->id}}','image-preview_{{$row->id}}',512,512);"/>
